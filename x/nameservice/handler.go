...
func handleMsgBuyName(ctx sdk.Context, keeper Keeper, msg MsgBuyName) sdk.Result {
	...
	if keeper.HasOwner(ctx, msg.NameID) { //if has owner, buy from the existing owner by sending from buyer to current owner
		_, err := keeper.coinKeeper.SendCoins(ctx, msg.Buyer, keeper.GetOwner(ctx, msg.NameID), msg.Bid)
		if err != nil {
			return sdk.ErrInsufficientCoins("Buyer does not have enough coins").Result()
		}
	} else { // if no owner, deduct bid amount from sender
